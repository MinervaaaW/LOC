...
from wan.runtime_config_freeloc import load_runtime_config
...
def generate(args):
    ...
    force_offload_model = runtime_config.get("generate", {}).get("force_offload_model")
    if force_offload_model is not None:
        args.offload_model = bool(force_offload_model)
        logging.info(
            f"offload_model is overridden by runtime config: {args.offload_model}.")
    if world_size > 1:
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            args.save_file = os.path.join(args.save_file, f"{formatted_prompt}_{formatted_time}" + suffix)
